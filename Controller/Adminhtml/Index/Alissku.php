    ...
    if (strlen($product_sku) > 0) {
        ...
        if (count($productSku) > 0) {
            foreach ($productSku as $alias_sku) {
                if ($alias_sku != "") {
                    // Get the parent SKUs from alias (can be multiple)
                    $aliasData = $this->datahelper->getAliasBySku($alias_sku);
                    
                    if (empty($aliasData) || !isset($aliasData[0]['sku'])) {
                        $insert_data = [
                            "sku" => $alias_sku,
                    ...
                    // Loop through each parent SKU found for this alias
                    foreach ($aliasData as $aliasRow) {
                        $sku = $aliasRow['sku'];
                        $all_alias_identifier = $aliasRow['all_alias_identifier'] ?? '';
                        ...
                        // Process the alias SKU for each parent SKU
                        $this->getApiData($alias_sku, $all_alias_identifier, $property_id, $collection_value, $select_attribute, $collection_slug_val, $sku);
                    }
                }
            }
        }
        ...
        try {
            ...
            if ($select_attribute == "image") {
                ...
                if (!empty($videovalue)) {
                    ...
                    $old_value_array = isset($videovalue[$product_sku_key]) ? $videovalue[$product_sku_key] : [];
                    $old_item_url = [];
                    $old_image_details = [];
                    ...
                    $new_value_array = json_encode($video_value, true);
